...
/**
 * Licence numbers deliberately kept OUT of the directory, for reasons that are
 * a business decision rather than a data one.
 *
 * This set exists because the exclusion has to live in the importer to be
 * durable. Commit faefed0 ("Removed Gaston #1 from sources", a Lovable edit)
 * deleted the record by hand from src/data/imported.generated.ts but left the
 * row in assisted-living-sacramento-area.csv — so the very next run of this
 * script silently republished it. A hand edit to an autogenerated file is not
 * a decision the codebase remembers.
 *
 * Add a licence number here only on an explicit owner instruction, and record
 * the reason. If a reason is unknown, say so rather than inventing one.
 */
const EXCLUDED_LICENSES = new Set([
  // Gaston Communities #1, 8131 Treecrest Ave, Fair Oaks. Removed by the owner
  // in commit faefed0 (2026). Reason not recorded in that commit; preserved
  // here so the removal survives regeneration. Confirm the reason and annotate.
  '345920060',
]);
...
/**
 * Jaccard similarity of two name-token sets: intersection over UNION.
 *
 * This replaces an intersection-over-min(size) score that produced false
 * positives so severe they put a wrong licence number on live community pages.
 * Dividing by the smaller set means a one-token name scores a perfect 1.0
 * against anything containing that token:
 *
 *     "GROVE HOME CARE"                    -> {GROVE}                       (HOME, CARE are stopwords)
 *     "Love and Serenity of Elk Grove III" -> {LOVE,SERENITY,ELK,GROVE,III}
 *     old: 1 / min(1,5) = 1.00   ->  matched, and it was a different building
 *     new: 1 / union 5  = 0.20   ->  rejected
 *
 * Union is the honest denominator: it penalises the tokens the candidate does
 * NOT share, which is exactly the signal the old score threw away.
 */
export function nameTokensJaccard(aTokens, bTokens) {
...
/**
 * Minimum distinctive tokens a name must retain after stopword removal before
 * its similarity score is trusted at all. Names like "SENIOR CARE HOME" reduce
 * to nothing; "GROVE HOME CARE" reduces to one generic token. Neither carries
 * enough signal to identify a facility.
 */
export const MIN_NAME_TOKENS = 2;

/** Jaccard floor for accepting a match whose street address already agrees. */
export const NAME_SCORE_FLOOR = 0.34;

/**
 * Higher Jaccard floor used on FULL names (stopwords retained) when stopword
 * removal left one side below MIN_NAME_TOKENS.
 */
export const FULL_NAME_SCORE_FLOOR = 0.6;

/** Name tokens with stopwords retained. */
export function allNameTokens(name) {
  return name.toUpperCase().replace(/[^A-Z0-9 ]+/g, ' ').split(/\s+/).filter(Boolean);
}

/**
 * Decide whether a CCLD roster row identifies the same facility as a curated
 * record.
 *
 * THE ADDRESS IS THE TRUTH TEST. A licensed facility is a building; two homes
 * can share a trade name and one home can be licensed under a name it does not
 * trade as, but a licence is tied to a street address. So a match REQUIRES the
 * normalised street addresses to agree, and the name score only guards against
 * matching a genuinely different operator that happens to occupy the same
 * address after a change of hands.
 *
 * Equivalently, and as the rule was specified: a match is hard-rejected
 * whenever the names disagree AND the addresses disagree. Requiring address
 * agreement is the stronger form of that rule.
 *
 * The old ZIP-plus-high-name-score path is gone. It existed to tolerate
 * curated address typos, but a ZIP covers thousands of homes, so it was the
 * path both real-world false positives travelled through. Every one of the 21
 * correct matches in the August 2026 audit agrees on address exactly; nothing
 * legitimate depended on it.
 */
export function matchesCurated(candidate, curated) {
  if (normAddress(candidate.streetAddress) !== normAddress(curated.street_address)) {
    return false;
  }

  const aTokens = normNameTokens(candidate.facilityName);
  const bTokens = normNameTokens(curated.name);

  // When stopword removal leaves too little on either side, the distinctive-
  // token score is not meaningful. "Sacramento Senior Living" reduces to
  // {SACRAMENTO} because SENIOR and LIVING are both stopwords, and "Senior
  // Care Home" reduces to nothing at all. Fall back to comparing the FULL
  // names, stopwords included, at a higher bar — two records at one address
  // whose complete names agree are the same facility, while two different
  // operators at one address are not rescued by this path.
  if (aTokens.length < MIN_NAME_TOKENS || bTokens.length < MIN_NAME_TOKENS) {
    return nameTokensJaccard(allNameTokens(candidate.facilityName), allNameTokens(curated.name))
      >= FULL_NAME_SCORE_FLOOR;
  }

  return nameTokensJaccard(aTokens, bTokens) >= NAME_SCORE_FLOOR;
}
...
function main() {
  ...
  // 1. Filter LICENSED + ON PROBATION, no closed_date, minus explicit exclusions.
  const inScope = data.filter((r) => {
    const status = r[ci('facility_status')];
    const closed = r[ci('closed_date')];
    if (EXCLUDED_LICENSES.has(r[ci('facility_number')])) return false;
  ...
  for (const r of inScope) {
    ...
    // Dedupe against curated. See matchesCurated: the street address must
    // agree, and the name score only guards against a different operator now
    // occupying the same building.
    //
    // A curated record can be claimed ONCE. Previously every matching row was
    // dropped from the imported list while `enrichmentMap[id] = ...`
    // overwrote, so when two roster rows both matched one curated record the
    // last row's licence was recorded and BOTH facilities disappeared from the
    // site. That is how #342700440, #342700828 and #342700201 — three real,
    // licensed Sacramento-area homes — came to be absent from the directory
    // entirely. Skipping already-claimed records means the second row falls
    // through and is published as its own facility, which is what it is.
    const match = curatedNameKeyIndex.find(
      (c) => !matchedCuratedIds.has(c.summary.id)
        && matchesCurated({ facilityName, streetAddress }, c.summary),
    )?.summary ?? null;
  ...
  for (const r of data) {
    ...
    for (const c of curatedNameKeyIndex) {
      if (matchesCurated({ facilityName, streetAddress }, c.summary)) {
...
// Only run the import when executed directly. The matcher functions above are
// exported so scripts/import-cdss.test.mjs can exercise them without
// regenerating any data files.
